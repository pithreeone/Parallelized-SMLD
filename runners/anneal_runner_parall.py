...
__all__ = ['AnnealRunnerParall']


class AnnealRunnerParall():
    ...
    def anneal_Langevin_dynamics_parall(self, scorenet, sigmas, grid_size=1, n_steps_epoch=100, step_lr=0.00002):
        # images = []
        print(len(sigmas), sigmas[0], sigmas[-1])
        
        # device
        device = self.config.device
        
        T = len(sigmas) # split the time t into T steps
        K = 10
        sigmas = torch.tensor(sigmas, device=device).view(10, 1, 1, 1)  # size: [10, 1, 1, 1]
        print(sigmas)
        # Sample initial condition from prio
        samples = torch.rand(T + 1, 1, 28, 28, device=device)
        s = torch.rand(T + 1, 1, 28, 28, device=device)
        # z = torch.randn_like(samples) * sigmas

        # Compute d(sigma^2)/dt
        time = torch.linspace(0, 9, 10, requires_grad=True)
        base = 0.01 ** (1/9)
        sigma = base ** time
        sigma_squared = sigma ** 2  # Element-wise square
        gradients = torch.autograd.grad(outputs=sigma_squared.sum(), inputs=time, create_graph=True)[0]
        print(gradients)

        step_size = 1
        with torch.no_grad():
            # Iterate k: K is the number of iteration of Picard
            for k in tqdm.tqdm(range(K)):

                prefix_sum_s = torch.zeros(T+1, 1, 28, 28, device=device)
                # prefix_sum_s[0] = samples[0]
                # Iterate t: T means split the time to T steps
                for t in range(1, T+1):
                    label = torch.ones(1, device=device).long() * (t - 1)
                    
                    s[t-1] = -scorenet(samples[t-1:t], label) * gradients[t-1] * step_size
                    # print(t-1, s[t-1, 0, :2])
                    if t == 1:
                        prefix_sum_s[t] = s[0]
                    else:
                        prefix_sum_s[t] = prefix_sum_s[t-1] + s[t-1]

                # print(samples.shape, prefix_sum_s.shape)
                samples = samples[0] + prefix_sum_s
        # return torch.clamp(samples, 0.0, 1.0).to('cpu')
        # print(samples[6])
        return samples
    ...
    def test(self):
        ...
        if self.config.data.dataset == 'MNIST':
            ...
            all_samples = self.anneal_Langevin_dynamics_parall(score, sigmas, grid_size, 100, 0.00002)
